...
router.post("/:project_id/:student_id", async (req, res) => {
  ...
  try {
    ...
    if (voteExistsResult && voteExistsResult[0] && voteExistsResult[0].count) {
      ...
      // Send notifications to each student associated with the project
      ...
      const notificationMessage = `User with id ${studentId} has upvoted your project.`;
      await Promise.all(students.map(student => createNotification(student, studentId, projectId, 'vote', notificationMessage)));
