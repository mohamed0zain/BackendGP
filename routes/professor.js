...
router.put("/accept/project/:project_id/:professor_id", isProfessor, async (req, res) => {
  try {
      ...
      const studentsQuery = "SELECT student_id FROM project_students WHERE project_id = ?";
      const studentRows = await conn.query(studentsQuery, [project_id]);
      const professorNameQuery = "SELECT professor_name FROM professor WHERE professor_id = ?";
      ...
      for (const student of students) {
          ...
          await createNotification(student, professor_id, project_id, 'project_status_update', notificationMessage);
      }
...
router.put("/project/assign-grades", async (req, res) => {
  try {
    ...
    await createNotification(student_id, professor_id, project_id, 'grade_update', notificationMessage);
